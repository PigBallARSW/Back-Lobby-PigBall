...
class TestLobbyController {
        ...
        void createLobby_ShouldReturnBadRequestOnError() throws Exception {
                when(lobbyService.createLobby(any(LobbyDTO.class)))
                                .thenThrow(new RuntimeException("Error al crear lobby"));
        ...
        void getAllLobbies_ShouldReturnListOfLobbies() throws Exception {
                ...
                List<LobbyDTO> lobbies = List.of(
                                new LobbyDTO("1", "Lobby1", "CreatorA", 4, LobbyStatus.WAITING_FOR_PLAYERS, false,
                                                Instant.now(), 100,
                                                100, Collections.emptyList(), Collections.emptyList(), "classic"),
                                new LobbyDTO("2", "Lobby2", "CreatorB", 6, LobbyStatus.FINISHED, true, Instant.now(),
                                                200, 200,
        ...
        void removeGame_ShouldReturnSuccessMessage() throws Exception {
                // Crea un LobbyDTO dummy para simular la respuesta
                LobbyDTO mockLobbyDTO = new LobbyDTO(
                                "1", "TestLobby", "Creator1", 8, LobbyStatus.WAITING_FOR_PLAYERS,
                                false, Instant.now(), 100, 100, Collections.emptyList(), Collections.emptyList(),
                                "classic");

                // Configura el mock para retornar el DTO simulado
                when(lobbyService.removeLobby(anyString())).thenReturn(mockLobbyDTO);
